...
- src/lib/notification.js:7-8 uses NEXT*PUBLIC*\* for WhatsApp API URL and access token.
- lib/xero/getAccessToken.js:11-13 uses NEXT*PUBLIC*\* for refresh token and client secret.
...
- src/app/api/v1/stock*transaction/create/route.js:146 parses the error string as "[, requested, available]", which swaps item_id and quantity.
  Impact: Error message shows incorrect values to users.
  Recommendation: Parse with [*, failedItemId, requested, available] or include a structured error object.

5. Medium - Logging path references undefined variable

- src/app/api/v1/uploads/lots/[...path]/route.js:331 references tab.id, but no tab variable exists in scope.
...
- src/app/api/v1/dashboard/route.js:111 returns null when year is "all" but month is specific.
- src/app/api/v1/dashboard/route.js:145 only applies date filters if dateRangeFilter is truthy.
...
- src/app/api/v1/dashboard/route.js:446 sorts on b.\_count - a.\_count, but \_count is an object with \_all.
...
- src/app/api/v1/search/route.js:30,45,70,81,153 runs multiple queries sequentially.
...
- src/app/api/v1/uploads/lots/[...path]/route.js:150 and src/app/mediauploads/[...path]/route.js:88 read entire files into memory.
