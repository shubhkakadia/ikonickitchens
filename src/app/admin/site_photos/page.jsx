...
export default function SitePhotosPage() {
    ...
    const handleFileSelect = async (lot, tabKind, event) => {
        ...
        try {
            ...
            if (heicFiles.length > 0) {
                toast.info(
                    `Converting ${heicFiles.length} HEIC file(s) to JPEG...`
                );
            }
                ...
                if (uploadedFiles.length > 0) {
                    ...
                    await Promise.all(
                        uploadedFiles.map(async (uploadedFile, index) => {
                            ...
                            const notes = fileKeys.length > index ? notesMap[fileKeys[index]] || "" : "";
    ...
    const handleSubmitCurrent = async () => {
        ...
        if (remainingFiles.length === 0) {
            ...
            const newIndex = currentIndex >= remainingFiles.length ? remainingFiles.length - 1 : currentIndex;
            setPendingUploads({
                ...pendingUploads,
                                    ...
                                    {isExpanded && (
                                        <div className="border-t border-gray-200">
                                            ...
                                            <div className="flex border-b border-gray-200 bg-gray-50">
                                                {getAllowedTabs().map((tabKind) => (
                                                    <button
                                                        ...
                                                        className={`flex-1 px-3 py-2.5 text-sm font-medium transition-colors ${activeTab === tabKind
                                            ...
                                            <div className="p-4">
                                                ...
                                                <div className="mb-4">
                                                    <label
                                                        className={`flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${uploading[`${lot.id}_${activeTab}`]
                                                    ...
                                                    return (
                                                        <div className="grid grid-cols-3 gap-2 mb-4">
                                                            {files.map((file) => {
                                                                ...
                                                                const hasNotes = fileNotes[file.id] && fileNotes[file.id].trim() !== "";
                                                                return (
                                                                    <div
                                                                        key={file.id}
                ...
                {selectedFile && (
                    ...
                    >
                        ...
                        >
                            ...
                            {/* File Preview */}
                            <div className="p-4">
                                {selectedFile.file_kind === "PHOTO" && getFileUrl(selectedFile) ? (
                                    <img
                                        src={getFileUrl(selectedFile)}
                                        alt={selectedFile.filename}
                                        className="w-full rounded-lg mb-4"
                                    />
                                ) : selectedFile.file_kind === "VIDEO" && getFileUrl(selectedFile) ? (
                                    <video
                                        src={getFileUrl(selectedFile)}
                                        controls
                                        playsInline
                            ...
                            <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-10">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-0.5">
                                        {pendingUploads.files.length} file(s) - {pendingUploads.currentIndex + 1} of {pendingUploads.files.length}
                                    </p>
                                </div>
                                <button
                                    ...
                                    {pendingUploads.files.map((fileItem, index) => {
                                        ...
                                        return (
                                            <div key={fileItem.id} className="space-y-4">
                                                ...
                                                {/* Action Buttons */}
                                                <div className={`flex gap-2 pt-2 ${pendingUploads.files.length > 1 ? "" : ""}`}>
                                                    {pendingUploads.files.length > 1 && (
                                                        <button
                                                            onClick={handleSubmitCurrent}
                                                            disabled={uploading[`${pendingUploads.lot.id}_${pendingUploads.tabKind}`]}
                                                        ...
                                                        >
                                                            {uploading[`${pendingUploads.lot.id}_${pendingUploads.tabKind}`] ? (
                                                                <>
                                                                    <Loader2 className="w-4 h-4 animate-spin" />
                                                                    <span>Uploading...</span>
                                                                </>
                                                    ...
                                                    >
                                                        {uploading[`${pendingUploads.lot.id}_${pendingUploads.tabKind}`] ? (
                                                            ...
                                                            <>
                                                                <Send className="w-4 h-4" />
                                                                <span>{pendingUploads.files.length > 1 ? "Submit All" : "Submit"}</span>
                                                            </>
                                                        )}
                                                    </button>
                                                </div>
